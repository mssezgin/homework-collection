...
#define CALLER_TYPE(c) ((c == 0) ? ("PP_GATHER") : ((c == 1) ? ("SS_SIT") : ((c == 2) ? ("SS_THROW") : ("UNKNOWN_CALLER"))))
...
void hw2_notify(enum hw2_actions action, unsigned id, unsigned i, unsigned j, unsigned c)
{
    ...
    // custom
    case CELL_LOCKED:       fprintf(stdout, " (%u, %u) is locked by             (id: %d) %s.\n", i, j, id, CALLER_TYPE(c)); break;
    case CELL_NOT_LOCKED:   fprintf(stdout, " (%u, %u) could not be locked by   (id: %d) %s.\n", i, j, id, CALLER_TYPE(c)); break;
    case CELL_UNLOCKED:     fprintf(stdout, " (%u, %u) is unlocked by           (id: %d) %s.\n", i, j, id, CALLER_TYPE(c)); break;
    case CELL_WAITING:      fprintf(stdout, " (%u, %u) is being waited by       (id: %d) %s.\n", i, j, id, CALLER_TYPE(c)); break;
    case CELL_WAITED:       fprintf(stdout, " (%u, %u) is available to          (id: %d) %s.\n", i, j, id, CALLER_TYPE(c)); break;
    }
