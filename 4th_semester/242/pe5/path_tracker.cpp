...
PathTracker::PathTracker()
    : final_x(0), final_y(0), displacement(0.f), distance(0.f) {
}
...
PathTracker::PathTracker(int *existing_path, int number_of_vectors)
    ...
    for (; number_of_vectors--; existing_path += 2) {
        *this += existing_path;
    }
}
...
PathTracker &PathTracker::operator+=(const int *new_vector) {
    ...
    this->calculate_distance(new_vector[0], new_vector[1]);
    this->calculate_displacement();
    return *this;
}
...
float PathTracker::calculate_displacement() {
    ...
    return (this->displacement = this->calculate_l2(0, 0, this->final_x, this->final_y));
}
...
void PathTracker::calculate_distance(int x, int y) {
    ...
    this->distance += this->calculate_l2(0, 0, x, y);
    this->final_x += x;
    this->final_y += y;
}
...
void PathTracker::summary() {
    ...
    std::cout << "Final Position: [" << this->final_x << "," << this->final_y
        << "] Displacement: " << this->displacement << " Distance: " << this->distance << std::endl;
}
